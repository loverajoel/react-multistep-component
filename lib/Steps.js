...
export default class Steps extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="steps-component">
        {
          this._printStepNavigator(children, currentStep, this.props.showNavigator)
        }
  ...
  _printStepNavigator(children, currentStep, showNavigator){
    return (
      showNavigator === true ?
    ...
    return (
      React.Children.map(children, (child, index) => {
        ...
        if(currentIndex === currentStep) {
          ...
          stepClass = 'visited-step';
        }

        let { customNavigator } = child.props;
        return (
          <li key={index} className={stepClass} onClick={() => {this._moveStep(index + 1)}}>
